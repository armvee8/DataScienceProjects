...
```{r}
nn <- neuralnet(f, data=phish.train.NN, hidden = c(3,4), linear.output=FALSE)
```
...
```{r}
colsOfInterestTest <- phish.test.NN %>% select(SSLfinal_State, URL_of_Anchor, web_traffic, having_Sub_Domain, Links_in_tags, Result)
```

Now we need to create predictions off of the test set.

```{r}
predicted.nn5vars <- neuralnet::compute(nn_5Vars, colsOfInterestTest)
```

Make a confusion matrix to evaluate.

```{r}
confusionMatrixNN<- confusionMatrix(round(predicted.nn5vars$net.result), colsOfInterestTest$Result)
